...
var Atrament = function () {
	function Atrament(selector, width, height, color) {
		...
		var mouseMove = function mouseMove(mousePosition) {
			...
			if(_this2.teacherMouseDown != true){
			...
			if (_this2.mouse.down) {
				...
				//sax stexa linum
				stream.submitInfo(mx,my,_this2.mouse);
				if(_this2.teacherMouseDown != true){
					_this2.draw(mx, my);
				}
			}
		...
		var mouseDown = function mouseDown(mousePosition) {
			hinvijak.push(canvas.toDataURL());
			vijak_count++;
			mousePosition.preventDefault();
			//update position just in case
				stream.SendMouseDown(true,_this2.mouse);
		...
		var mouseUp = function mouseUp(mousePosition) {
			if(_this2.mouse.down){
				mousePosition.preventDefault();
				setTimeout(function(){stream.SendMouseDown(false)},200);
	...
	_createClass(Atrament, [{
		key: 'draw',
		value: function draw(mX, mY,_mouse = 0,teacher=0) {
			//jnjel stex
			if(this.teacherMouseDown==true && !teacher){
				return;
			}
			if(_mouse == 0){
			...
			*/
			...
			this.mouse.px = mouse.x;
			this.mouse.py = mouse.y;
		...
		key: 'setmouseDown',
		value: function setmouseDown(data) {
			this.teacherMouseDown = data.data;
			if (data.data==true) {
				console.log(data.mouse)
				this.mouse.x = data.mouse.x;
				this.mouse.y = data.mouse.y;

				this.context.closePath();
				this.mouse.down = false;
			};
			//return this.mouse.down = data;
		}
	}, {
		key: 'fill',
		value: function fill() {
...
function _atrament(selector, width, height, color) {
	var atrament = new Atrament(selector, width, height, color);
	return atrament;
}
